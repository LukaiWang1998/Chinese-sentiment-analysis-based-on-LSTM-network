...
def train_lstm(input_dim, embedding_weights, x_train, y_train, x_test, y_test):
    ...
    model.fit(x_train, y_train, batch_size=batch_size, epochs=epoch_time, verbose=1,validation_split=0.2,
              callbacks=[
                  ...
                  keras.callbacks.ModelCheckpoint('../model/posneg+jg训练 唐山测试.h5', monitor='val_acc',
    ...
    print "Evaluate..."
    tangshan_model = load_model('../model/posneg+jg训练 唐山测试.h5',
                                compile=False)
...
print X_Vec1.shape
print y1.shape
X_Vec2, y2 = loadfile2()#jg
print X_Vec2.shape
print y2.shape
X_Vec3, y3 = loadfile1()#唐山
print X_Vec3.shape
print y3.shape
...
X_Vec = np.concatenate((X_Vec1,X_Vec2,X_Vec3))
...
x_train, x_test, y_train, y_test = train_test_split(index2, y, test_size=0.2,random_state=42)
